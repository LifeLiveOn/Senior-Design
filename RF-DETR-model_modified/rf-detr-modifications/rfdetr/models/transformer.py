...
def as_int(x): return int(x.item()) if torch.is_tensor(x) else int(x)
...
def gen_encoder_output_proposals(memory, memory_padding_mask, spatial_shapes, unsigmoid=True):
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        if memory_padding_mask is not None:
            ...
            h = as_int(H_)
            w = as_int(W_)
            mask_flatten_ = memory_padding_mask[:, cur:(
                cur + h * w)].view(N_, h, w, 1)
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        if self.two_stage:
            ...
            for g_idx in range(group_detr):
                ...
                topk = min(self.num_queries,
                           enc_outputs_class_unselected_gidx.shape[-2])
        ...
        if self.dec_layers > 0:
            ...
            if self.two_stage:
                ...
                if self.bbox_reparam:
                    refpoint_embed_cxcy = refpoint_embed_ts_subset[...,
                                                                   :2] * refpoint_embed_ts[..., 2:]
                    refpoint_embed_cxcy = refpoint_embed_cxcy + \
                        refpoint_embed_ts[..., :2]
                    refpoint_embed_wh = refpoint_embed_ts_subset[..., 2:].exp(
                    ) * refpoint_embed_ts[..., 2:]
